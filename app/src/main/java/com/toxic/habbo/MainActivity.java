...
public class MainActivity extends Activity {
    ...
    private void buildUi() {
        ...
        searchCard.addView(searchInput, lp(-1, dp(42), 0, 0, 0, 10));
    ...
    private ProfileResult loadProfile(String nick) throws Exception {
        ...
        if (!r.uniqueId.isEmpty()) {
            ...
            r.photos = fetchOfficialPhotos(r.uniqueId);

            // Carrega um primeiro lote rápido. O restante fica paginado/assíncrono pelo backend do Toxic.
            r.previousMottos = fetchAll(r.uniqueId, "previous-mottos", null, 100, 3);
            r.previousStyles = fetchAll(r.uniqueId, "previous-styles", null, 100, 3);
            if (r.friends.isEmpty()) r.friends = fetchAll(r.uniqueId, "friends", "friends", 100, 3);
            r.oldFriends = fetchAll(r.uniqueId, "previous-friends", null, 30, 5);
            if (r.rooms.isEmpty()) r.rooms = fetchAll(r.uniqueId, "rooms", "rooms", 100, 3);
            r.oldRooms = fetchAll(r.uniqueId, "previous-rooms", "rooms", 100, 3);
            if (r.groups.isEmpty()) r.groups = fetchAll(r.uniqueId, "groups", "groups", 100, 3);
        }
        return r;
    }
    ...
    private void renderProfile(ProfileResult r) {
        ...
        FrameLayout.LayoutParams cp = new FrameLayout.LayoutParams(-2, dp(38), Gravity.BOTTOM | Gravity.CENTER_HORIZONTAL);
        ...
        badges.addView(profileBadge(r.privateProfile ? "Privado" : "Público", r.privateProfile ? "lock" : "dot", r.privateProfile ? red : purple));
    ...
    private LinearLayout profileBadge(String label, String icon, int color) {
        ...
        TextView tv = text(label, 15, Color.WHITE, true);
    ...
    private void addSelectedBadges(ArrayList<JSONObject> list) {
        ...
        for (int i=0; i<Math.min(list.size(), 5); i++) {
            ...
            LinearLayout.LayoutParams p = new LinearLayout.LayoutParams(dp(50), dp(50)); p.rightMargin = dp(8); row.addView(img, p);
    ...
    private TextView tabButton(String s, boolean active) {
        ...
        LinearLayout.LayoutParams p = new LinearLayout.LayoutParams(-2, dp(50)); p.rightMargin = dp(8); v.setLayoutParams(p); return v;
    }
    ...
    private void renderFriendsPage(LinearLayout content, ArrayList<JSONObject> data, int page, int per, boolean removed) {
        ...
        for (int i=start; i<end; i+=2) {
            ...
            row.addView(friendCard(data.get(i), removed), new LinearLayout.LayoutParams(0, dp(136), 1));
            if (i+1<end) { LinearLayout.LayoutParams p = new LinearLayout.LayoutParams(0, dp(136), 1); p.leftMargin = dp(12); row.addView(friendCard(data.get(i+1), removed), p); }
            else { Space sp = new Space(this); LinearLayout.LayoutParams p = new LinearLayout.LayoutParams(0, dp(136), 1); p.leftMargin=dp(12); row.addView(sp,p); }
        }
    }
    ...
    private LinearLayout friendCard(JSONObject f, boolean removed) {
        ...
        FrameLayout headWrap = new FrameLayout(this); card.addView(headWrap, new LinearLayout.LayoutParams(-1, dp(62)));
    ...
    private TextView pageButton(String s, boolean enabled) { TextView v = text(s, 20, enabled?Color.WHITE:Color.argb(70,255,255,255), true); v.setGravity(Gravity.CENTER); v.setBackground(round(Color.argb(14,255,255,255), dp(14), Color.argb(24,255,255,255), 1)); LinearLayout.LayoutParams p = new LinearLayout.LayoutParams(dp(50), dp(50)); p.setMargins(dp(6),0,dp(6),0); v.setLayoutParams(p); return v; }
    ...
    private LinearLayout groupRow(JSONObject g) {
        ...
        ImageView img = new ImageView(this); img.setScaleType(ImageView.ScaleType.CENTER_INSIDE); row.addView(img, new LinearLayout.LayoutParams(dp(50), dp(50)));
    ...
    public class IconView extends View {
        ...
        @Override protected void onDraw(Canvas c) { super.onDraw(c); float w=getWidth(), h=getHeight(), cx=w/2, cy=h/2; p.setStyle(Paint.Style.STROKE); p.setStrokeWidth(dp(3)); p.setStrokeCap(Paint.Cap.ROUND); p.setColor(Color.WHITE);
            ...
            if ("lock".equals(type)) { RectF body = new RectF(cx-dp(10), cy-dp(1), cx+dp(10), cy+dp(14)); c.drawRoundRect(body, dp(3), dp(3), p); c.drawArc(new RectF(cx-dp(8), cy-dp(14), cx+dp(8), cy+dp(6)), 200, 140, false, p); return; }
