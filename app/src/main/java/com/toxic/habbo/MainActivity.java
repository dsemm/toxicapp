...
public class MainActivity extends Activity {
    ...
    private ProfileResult loadProfile(String nick) throws Exception {
        ...
        if (!r.uniqueId.isEmpty()) {
            ...
            r.photos = mergeLists(fetchOfficialPhotos(r.uniqueId), fetchAll(r.uniqueId, "photos", null, 100, 3));
    ...
    private LinearLayout statRow(String icon, String label, String value) {
        ...
        LinearLayout.LayoutParams rp = lp(-1, dp(74), 0, 0, 0, 10);
        ...
        LinearLayout.LayoutParams tp = new LinearLayout.LayoutParams(0, -2, 1); tp.leftMargin = dp(12); row.addView(texts, tp);
        texts.addView(text(label, 14, Color.argb(190,255,255,255), false));
        texts.addView(text(value == null || value.isEmpty() ? "—" : value, 17, Color.WHITE, true));
        return row;
    }
    ...
    private void addPreviousNames(ArrayList<JSONObject> list) {
        ...
        c.addView(sv, lp(-1, dp(Math.min(230, Math.max(90, 58 * Math.min(list.size(), 4)))), 0, 0, 0, 0));
    ...
    private ArrayList<JSONObject> filterPreviousNickSuggestions(JSONObject suggest, String query) {
        ...
        for (JSONObject user : users) {
            ...
            if (getExactPreviousNameMatch(user, q) != null) out.add(user);
            if (out.size() >= 6) break;
        }
        return out;
    }
