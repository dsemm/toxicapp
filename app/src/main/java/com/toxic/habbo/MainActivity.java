...
public class MainActivity extends Activity {
    ...
    private void buildUi() {
        ...
        searchCard.addView(searchInput, lp(-1, dp(44), 0, 0, 0, 12));
        ...
        searchCard.addView(searchBtn, lp(-1, dp(70), 0, 0, 0, 0));
    ...
    private ProfileResult loadProfile(String nick) throws Exception {
        ...
        if (!r.uniqueId.isEmpty()) {
            ...
            r.previousMottos = fetchAll(r.uniqueId, "previous-mottos", null, 100, 10);
            r.previousStyles = fetchAll(r.uniqueId, "previous-styles", null, 100, 10);
            r.photos = fetchAll(r.uniqueId, "photos", null, 100, 10);
            r.friends = fetchAll(r.uniqueId, "friends", "friends", 100, 10);
            r.oldFriends = fetchAll(r.uniqueId, "previous-friends", null, 30, 12);
            r.rooms = fetchAll(r.uniqueId, "rooms", "rooms", 100, 10);
            r.oldRooms = fetchAll(r.uniqueId, "previous-rooms", "rooms", 100, 10);
            r.groups = fetchAll(r.uniqueId, "groups", "groups", 100, 10);
        }
        return r;
    }
    ...
    private void renderProfile(ProfileResult r) {
        ...
        avatar.setPadding(dp(20), dp(18), dp(20), dp(18));
        ...
        FrameLayout.LayoutParams cp = new FrameLayout.LayoutParams(-2, dp(54), Gravity.BOTTOM | Gravity.CENTER_HORIZONTAL);
        cp.bottomMargin = dp(12);
        avatarFrame.addView(controls, cp);
        controls.addView(roundIconButton("←"));
        controls.addView(roundIconButton("👕"));
        controls.addView(roundIconButton("→"));
    ...
    private TextView roundIconButton(String label) {
        TextView v = text(label, 24, Color.WHITE, true);
        v.setGravity(Gravity.CENTER);
        v.setBackground(round(Color.argb(45,255,255,255), dp(16), Color.argb(50,255,255,255), 1));
        LinearLayout.LayoutParams p = new LinearLayout.LayoutParams(dp(54), dp(54));
    ...
    private LinearLayout statRow(String icon, String label, String value) {
        ...
        LinearLayout.LayoutParams rp = lp(-1, dp(92), 0, 0, 0, 14);
        ...
        LinearLayout.LayoutParams tp = new LinearLayout.LayoutParams(0, -2, 1); tp.leftMargin = dp(16); row.addView(texts, tp);
        texts.addView(text(label, 17, Color.argb(190,255,255,255), false));
        texts.addView(text(value == null || value.isEmpty() ? "—" : value, 21, Color.WHITE, true));
        return row;
    }

    private void addSelectedBadges(ArrayList<JSONObject> list) {
        if (list.isEmpty()) return;
        LinearLayout c = sectionCard("Emblemas selecionados", list.size(), true);
        LinearLayout row = new LinearLayout(this); row.setOrientation(LinearLayout.HORIZONTAL); row.setGravity(Gravity.CENTER); c.addView(row, lp(-1, -2, 0, 0, 0, 0));
        for (int i=0; i<Math.min(list.size(), 5); i++) {
            JSONObject b = list.get(i); String code = firstText(b, "code", "badgeCode");
            ImageView img = new ImageView(this); img.setScaleType(ImageView.ScaleType.CENTER_INSIDE); img.setBackground(round(Color.argb(24,255,255,255), dp(12), Color.argb(28,255,255,255),1));
            LinearLayout.LayoutParams p = new LinearLayout.LayoutParams(dp(58), dp(58)); p.rightMargin = dp(8); row.addView(img, p);
    ...
    private void addPreviousNames(ArrayList<JSONObject> list) {
        ...
        for (int i=0; i<Math.min(list.size(), 20); i++) {
            ...
            c.addView(mottoItem(n.isEmpty()?"Nome anterior":n, niceDate(d)));
        }
    }
    ...
    private void addPreviousMottos(ArrayList<JSONObject> list) {
        ...
        for (int i=0; i<Math.min(list.size(), 5); i++) {
            ...
            String d = firstText(o, "changedAt", "date", "createdAt", "timestamp", "creationTime");
    ...
    private LinearLayout clothingRow(JSONObject o) {
        ...
        ImageView img = new ImageView(this); img.setScaleType(ImageView.ScaleType.CENTER_INSIDE); row.addView(img, new LinearLayout.LayoutParams(dp(48), dp(48)));
        String icon = firstText(o, "iconUrl", "imageUrl", "url", "thumbnail"); if (!icon.isEmpty()) loadImage(img, icon);
        ...
        String name = firstText(o, "name", "publicName", "classname", "code"); String code = firstText(o, "code", "classname", "category", "id");
        txt.addView(text(name.isEmpty()?"Peça":name, 15, Color.WHITE, true)); txt.addView(text(code, 13, muted, false));
        return row;
    }
    ...
    private void addPhotos(ArrayList<JSONObject> list) {
        ...
        for (int i=0; i<Math.min(list.size(), 24); i++) {
            JSONObject o = list.get(i);
            String url = firstText(o, "url", "previewUrl", "imageUrl", "photoUrl", "largeUrl");
            String date = niceDate(firstText(o, "date", "createdAt", "creationTime", "timestamp"));
            ...
            ImageView img = new ImageView(this); img.setScaleType(ImageView.ScaleType.CENTER_CROP); box.addView(img, new LinearLayout.LayoutParams(-1, dp(112)));
            TextView dt = text(date, 12, Color.argb(190,255,255,255), false); dt.setGravity(Gravity.CENTER); box.addView(dt, lp(-1,-2,0,8,0,0));
            if (!url.isEmpty()) { loadImage(img, url); final String finalUrl = url; box.setOnClickListener(v -> openUrl(finalUrl)); }
        }
    }
    ...
    private TextView tabButton(String s, boolean active) {
        TextView v = text(s, 18, active ? Color.WHITE : Color.argb(150,255,255,255), true); v.setGravity(Gravity.CENTER); v.setPadding(dp(16),0,dp(16),0); v.setBackground(tabBg(active));
        LinearLayout.LayoutParams p = new LinearLayout.LayoutParams(-2, dp(58)); p.rightMargin = dp(10); v.setLayoutParams(p); return v;
    }
    private Drawable tabBg(boolean active) { return active ? grad(dp(16), purple2, Color.rgb(166, 42, 235)) : round(Color.argb(12,255,255,255), dp(16), Color.argb(28,255,255,255), 1); }
    ...
    private void renderFriendsPage(LinearLayout content, ArrayList<JSONObject> data, int page, int per, boolean removed) {
        ...
        for (int i=start; i<end; i+=2) {
            ...
            row.addView(friendCard(data.get(i), removed), new LinearLayout.LayoutParams(0, dp(190), 1));
            if (i+1<end) { LinearLayout.LayoutParams p = new LinearLayout.LayoutParams(0, dp(190), 1); p.leftMargin = dp(12); row.addView(friendCard(data.get(i+1), removed), p); }
            else { Space sp = new Space(this); LinearLayout.LayoutParams p = new LinearLayout.LayoutParams(0, dp(190), 1); p.leftMargin=dp(12); row.addView(sp,p); }
        }
    }

    private LinearLayout friendCard(JSONObject f, boolean removed) {
        LinearLayout card = new LinearLayout(this); card.setOrientation(LinearLayout.VERTICAL); card.setGravity(Gravity.CENTER); card.setPadding(dp(10),dp(10),dp(10),dp(10)); card.setBackground(round(Color.argb(20,255,255,255), dp(18), Color.argb(25,255,255,255), 1));
        ...
        FrameLayout headWrap = new FrameLayout(this); card.addView(headWrap, new LinearLayout.LayoutParams(-1, dp(78)));
        ImageView head = new ImageView(this); head.setScaleType(ImageView.ScaleType.CENTER_INSIDE); headWrap.addView(head, new FrameLayout.LayoutParams(-1, -1));
        if (!fig.isEmpty()) loadImage(head, avatarHead(fig));
        if (isToday(date) && !removed) {
            TextView novo = text("NOVO", 11, Color.WHITE, true); novo.setGravity(Gravity.CENTER); novo.setBackground(grad(dp(999), Color.rgb(31,184,106), Color.rgb(54,210,127))); FrameLayout.LayoutParams np = new FrameLayout.LayoutParams(dp(70), dp(28), Gravity.TOP|Gravity.CENTER_HORIZONTAL); headWrap.addView(novo,np);
        }
        ...
        TextView name = habboText(n, 17, true); name.setGravity(Gravity.CENTER); name.setMaxLines(1); name.setEllipsize(TextUtils.TruncateAt.END); card.addView(name, lp(-1,-2,0,8,0,6));
        TextView d = text(niceDate(date), 13, Color.argb(185,255,255,255), false); d.setGravity(Gravity.CENTER); card.addView(d, lp(-1,-2,0,0,0,0));
    ...
    private void renderPager(LinearLayout content, int total, int per, int[] page, Runnable rerender) {
        ...
        TextView label = text("Página " + page[0] + " de " + totalPages, 18, Color.WHITE, true); label.setGravity(Gravity.CENTER); content.addView(label, lp(-1,-2,0,6,0,12));
    ...
    private TextView pageButton(String s, boolean enabled) { TextView v = text(s, 20, enabled?Color.WHITE:Color.argb(70,255,255,255), true); v.setGravity(Gravity.CENTER); v.setBackground(round(Color.argb(14,255,255,255), dp(14), Color.argb(24,255,255,255), 1)); LinearLayout.LayoutParams p = new LinearLayout.LayoutParams(dp(58), dp(58)); p.setMargins(dp(6),0,dp(6),0); v.setLayoutParams(p); return v; }
    ...
    private LinearLayout roomRow(JSONObject room) {
        ...
        ImageView img = new ImageView(this); img.setScaleType(ImageView.ScaleType.CENTER_CROP); img.setBackground(round(Color.argb(25,255,255,255), dp(12), Color.argb(20,255,255,255),1)); row.addView(img, new LinearLayout.LayoutParams(dp(112), dp(78)));
        String image = firstText(room, "imageUrl", "thumbnail", "url", "roomImage"); if (!image.isEmpty()) loadImage(img, image);
        LinearLayout txt = new LinearLayout(this); txt.setOrientation(LinearLayout.VERTICAL); LinearLayout.LayoutParams tp = new LinearLayout.LayoutParams(0,-2,1); tp.leftMargin=dp(12); row.addView(txt,tp);
        txt.addView(text(firstText(room,"name","roomName","caption","title").isEmpty()?"Quarto":firstText(room,"name","roomName","caption","title"), 16, Color.WHITE, true));
        ...
        String tag = firstText(room,"category","tag","description"); if(!tag.isEmpty()) txt.addView(pill(tag, purple));
        return row;
    }
    ...
    private LinearLayout groupRow(JSONObject g) {
        ...
        ImageView img = new ImageView(this); img.setScaleType(ImageView.ScaleType.CENTER_INSIDE); row.addView(img, new LinearLayout.LayoutParams(dp(58), dp(58)));
        String badge = firstText(g,"badgeCode","code"); if(!badge.isEmpty()) loadImage(img,"https://images.habbo.com/c_images/Badgeparts/generated/badge_"+enc(badge)+".png");
        LinearLayout txt = new LinearLayout(this); txt.setOrientation(LinearLayout.VERTICAL); LinearLayout.LayoutParams tp=new LinearLayout.LayoutParams(0,-2,1); tp.leftMargin=dp(12); row.addView(txt,tp);
        txt.addView(text(firstText(g,"name","groupName").isEmpty()?"Grupo":firstText(g,"name","groupName"), 17, Color.WHITE, true));
        String desc=firstText(g,"description","desc"); if(!desc.isEmpty()) txt.addView(text(desc, 14, Color.argb(220,255,255,255), false));
    ...
    private LinearLayout sectionCard(String title, int count, boolean showTitle) {
        ...
        if (showTitle && title != null) {
            TextView t = text(title + " (" + count + ")", 22, Color.WHITE, true); c.addView(t, lp(-1, -2, 0, 0, 0, 14));
        }
        return c;
    }
    ...
    private void setLoading(boolean loading, String message) { searchBtn.setEnabled(!loading); searchBtn.setText(loading ? "..." : "Pesquisar"); progress.setVisibility(loading ? View.VISIBLE : View.GONE); statusText.setText(message == null ? "" : message); }

    private JSONObject getJson(String u) throws Exception { HttpURLConnection c = (HttpURLConnection)new URL(u).openConnection(); c.setConnectTimeout(12000); c.setReadTimeout(24000); c.setRequestProperty("Accept", "application/json, text/plain, */*"); c.setRequestProperty("User-Agent", "ToxicHabboApp/2.0 Android"); int code = c.getResponseCode(); InputStream is = code >= 200 && code < 300 ? c.getInputStream() : c.getErrorStream(); String body = readAll(is); if (code < 200 || code >= 300 || body == null || body.trim().isEmpty()) throw new IOException("HTTP " + code); return new JSONObject(body); }
    ...
    private void loadImage(ImageView view, String url) { if (url == null || url.isEmpty()) return; executor.execute(() -> { try { HttpURLConnection c = (HttpURLConnection)new URL(url).openConnection(); c.setConnectTimeout(12000); c.setReadTimeout(22000); c.setRequestProperty("User-Agent", "ToxicHabboApp/2.0 Android"); Bitmap bmp = BitmapFactory.decodeStream(c.getInputStream()); runOnUiThread(() -> { if (bmp != null) view.setImageBitmap(bmp); }); } catch (Exception ignored) {} }); }
    ...
    private void addUnique(ArrayList<JSONObject> out, HashSet<String> seen, ArrayList<JSONObject> src) { for (JSONObject o : src) { String key = firstText(o, "id", "name", "figureString", "motto", "code", "uniqueId") + o.toString().hashCode(); if (seen.add(key)) out.add(o); } }
    ...
    private String niceDate(String in) {
        ...
        try {
            ...
            if (s.matches("^\\d{10,13}$")) {
                ...
                String[] patterns = {"yyyy-MM-dd'T'HH:mm:ss.SSSZ", "yyyy-MM-dd'T'HH:mm:ssZ", "yyyy-MM-dd HH:mm:ss", "yyyy-MM-dd"};
                Date parsed = null;
                for (String p: patterns) { try { SimpleDateFormat f = new SimpleDateFormat(p, Locale.US); f.setTimeZone(TimeZone.getTimeZone("UTC")); parsed = f.parse(iso); break; } catch(Exception ignored){} }
    ...
    public class IconView extends View {
        ...
        @Override protected void onDraw(Canvas c) { super.onDraw(c); float w=getWidth(), h=getHeight(), cx=w/2, cy=h/2; p.setStyle(Paint.Style.STROKE); p.setStrokeWidth(dp(3)); p.setStrokeCap(Paint.Cap.ROUND); p.setColor(Color.WHITE);
