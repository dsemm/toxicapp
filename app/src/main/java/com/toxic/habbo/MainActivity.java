...
public class MainActivity extends Activity {
    ...
    private void renderProfile(ProfileResult r) {
        ...
        FrameLayout.LayoutParams cp = new FrameLayout.LayoutParams(-2, dp(44), Gravity.BOTTOM | Gravity.CENTER_HORIZONTAL);
    ...
    private TextView roundIconButton(String label) {
        TextView v = text("shirt".equals(label) ? "" : label, 22, Color.WHITE, true);
        v.setGravity(Gravity.CENTER);
        v.setBackground(round(Color.argb(45,255,255,255), dp(12), Color.argb(50,255,255,255), 1));
    ...
    private LinearLayout statRow(String icon, String label, String value) {
        ...
        if ("status".equals(icon)) {
            ...
            iv.setImageResource("Online".equals(value) ? R.drawable.online : R.drawable.offline);
    ...
    private TextView tabButton(String s, boolean active) {
        TextView v = habboText(s, 18, true); v.setTextColor(active ? Color.WHITE : Color.argb(150,255,255,255)); v.setGravity(Gravity.CENTER); v.setPadding(dp(16),0,dp(16),0); v.setBackground(tabBg(active));
    ...
    private void renderFriendsPage(LinearLayout content, ArrayList<JSONObject> data, int page, int per, boolean removed) {
        ...
        for (int i=start; i<end; i+=2) {
            ...
            row.addView(friendCard(data.get(i), removed), new LinearLayout.LayoutParams(0, dp(156), 1));
            if (i+1<end) { LinearLayout.LayoutParams p = new LinearLayout.LayoutParams(0, dp(156), 1); p.leftMargin = dp(12); row.addView(friendCard(data.get(i+1), removed), p); }
            else { Space sp = new Space(this); LinearLayout.LayoutParams p = new LinearLayout.LayoutParams(0, dp(156), 1); p.leftMargin=dp(12); row.addView(sp,p); }
        }
    }

    private LinearLayout friendCard(JSONObject f, boolean removed) {
        LinearLayout card = new LinearLayout(this); card.setOrientation(LinearLayout.VERTICAL); card.setGravity(Gravity.CENTER); card.setPadding(dp(8),dp(8),dp(8),dp(8)); card.setBackground(round(Color.argb(20,255,255,255), dp(18), Color.argb(25,255,255,255), 1));
        ...
        FrameLayout headWrap = new FrameLayout(this); card.addView(headWrap, new LinearLayout.LayoutParams(-1, dp(76)));
        ...
        if (isToday(date) && !removed) {
            TextView novo = text("NOVO", 10, Color.WHITE, true); novo.setGravity(Gravity.CENTER); novo.setBackground(grad(dp(999), Color.rgb(31,184,106), Color.rgb(54,210,127))); FrameLayout.LayoutParams np = new FrameLayout.LayoutParams(dp(56), dp(22), Gravity.TOP|Gravity.CENTER_HORIZONTAL); headWrap.addView(novo,np);
        }
        ...
        TextView name = habboText(n, 16, true); name.setGravity(Gravity.CENTER); name.setMaxLines(1); name.setEllipsize(TextUtils.TruncateAt.END); card.addView(name, lp(-1,-2,0,8,0,6));
    ...
    private LinearLayout roomRow(JSONObject room) {
        ...
        String image = findImageUrlDeep(room); if (!image.isEmpty()) loadImage(img, image); else img.setImageDrawable(new PlaceholderDrawable("room"));
        LinearLayout txt = new LinearLayout(this); txt.setOrientation(LinearLayout.VERTICAL); LinearLayout.LayoutParams tp = new LinearLayout.LayoutParams(0,-2,1); tp.leftMargin=dp(12); row.addView(txt,tp);
        ...
        String tag = firstText(room,"category","tag","description"); if(!tag.isEmpty()) { TextView tg = pill(tag, purple); tg.setMaxLines(1); tg.setEllipsize(TextUtils.TruncateAt.END); txt.addView(tg); }
        return row;
    }
    ...
    private LinearLayout groupRow(JSONObject g) {
        ...
        String badge = firstText(g,"badgeCode","code"); if(!badge.isEmpty()) loadImage(img,"https://images.habbo.com/c_images/Badgeparts/generated/badge_"+enc(badge)+".png"); else img.setImageDrawable(new PlaceholderDrawable("groups"));
        LinearLayout txt = new LinearLayout(this); txt.setOrientation(LinearLayout.VERTICAL); LinearLayout.LayoutParams tp=new LinearLayout.LayoutParams(0,-2,1); tp.leftMargin=dp(12); row.addView(txt,tp);
    ...
    private LinearLayout sectionCard(String title, int count, boolean showTitle) {
        ...
        if (showTitle && title != null) {
            TextView t = habboText(title + " (" + count + ")", 22, true); c.addView(t, lp(-1, -2, 0, 0, 0, 14));
        }
        return c;
    }
    ...
    private void setLoading(boolean loading, String message) {
        ...
        searchBtn.setText(loading ? "Buscando..." : "Pesquisar");
        progress.setVisibility(View.GONE);
    ...
    private void showLoadingSkeleton(String message) {
        ...
        resultWrap.addView(c, lp(-1, -2, 0, 0, 0, 18));
        TextView title = habboText(message, 18, true); title.setGravity(Gravity.CENTER); c.addView(title, lp(-1,-2,0,0,0,14));
    ...
    public class ShirtDrawable extends Drawable {
        ...
        @Override public void draw(Canvas c) {
            ...
            c.drawRoundRect(new RectF(x,y,x+w,y+h), dp(12), dp(12), p);
            p.setColor(Color.rgb(235, 64, 72));
            Path shirt = new Path();
